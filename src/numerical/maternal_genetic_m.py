...
class IterM:
    ...
    def __init__(self, Gaa, Gbb, Gmm, omega_b_2, omega_m_2, shift=True, freq=0, tau=0.25, delta=10, init_g=0.001,init_b=0.001,init_m=0.001):
        ...
        # initialization
        self.mbar[0] = self.mbar[1] = self.init_m
        self.abar[0] = self.abar[1] = self.init_g
        self.bbar[0] = self.bbar[1] = self.init_b
        self.sigz[0] = self.sigz[1] = self.Gaa + self.Gbb * self.epsinit**2 + self.Gmm * self.epsinit**2 + self.sigma_e2
        self.zsbar[0] = self.zsbar[1]  = 0.0
        self.zbar[0] = self.zbar[1]  = 0.0
        self.gmzstar[0] = self.gmzstar[1] = 0.0
        self.epstT[0] = self.epsinit
        self.epst[0] = self.epsinit
    ...
    def run(self):
        ...
        for t in range(1,self.nrp-2):
            ...
            sigma_z2 = self.Gaa + self.Gbb*epstT**2.0 + self.Gmm*(self.Gaa+self.abar[t]**2) + self.Gaa * self.mbar[t] * (1.0 + self.mbar[t]) + self.sigma_e2
            ...
            # update \bar{z}_{t}
            self.zbar[t] = self.abar[t] + self.bbar[t]*epstT + self.mbar[t] + self.mbar[t]*self.abar[t]
            ...
            mat1 = -1.0 / self.omega_z_2 * np.array([[tmp * (1+self.mbar[t])], [tmp * epstT], [tmp * self.abar[t]]])
            
            dsigm = self.Gaa*(1+2*self.mbar[5])
            dsiga = 2*self.Gmm*self.abar[t]
            mat2 = 0.5*(-1.0/self.omega_z_2)*np.array([[dsiga], [0], [dsigm]])

            mat3=-1.0/self.omega_z_2*np.array([[0], [self.omega_z_2 * self.bbar[t]/self.omega_b_2], [self.omega_z_2*self.mbar[t]/self.omega_m_2]]);
            beta = mat1+mat2+mat3;
            ...
            self.abar[t+1] = self.abar[t] + delta_change[0,0]
            self.bbar[t+1] = self.bbar[t] + delta_change[1,0]
            self.mbar[t+1] = self.mbar[t] + delta_change[2,0]
