...
%%%%%%%%%%%% calculate sigma_z2 %%%%%%%%%%%%%%%%%%
sigma_z2(t)=Gaa+Gbb*eps_tT^2.0+Gmm*(Gaa+abar(t)^2.0)+Gaa*mbar(t)*(1+mbar(t))+sigma_e2;
sigma_z2(t)=max(0,sigma_z2(t));
%*%*%*%*%*%*%*%* calculate gamma
...
tmp=zbar(t)-theta;
adptn(t)=tmp;
mat1=-1/omega2*[tmp*(1+mbar(t)); tmp*eps_tT; tmp*abar(t)];
dsigm=Gaa*(1+2*mbar(t));
dsiga=2*Gmm*abar(t);
mat2=0.5*(-1/omega2)*[dsiga; 0; dsigm];
...
writetable(the_table, "output_matlab_run.txt", "Delimiter", ";", "WriteRowNames",false)
...
%subplot(3,2,6);semilogx(adptn);xlabel('generations','fontsize',14);ylabel('adaptation','fontsize',14); axis tight;
